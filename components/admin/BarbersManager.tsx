...
const ROLE_BADGE: Record<UserRole, string> = {
  customer: 'border-gray-200 text-gray-500 bg-gray-100',
  barber:   'border-brand-300 text-brand-600 bg-brand-50',
  admin:    'border-ocean-300 text-ocean-600 bg-ocean-50',
};

const ROLE_OPTIONS: UserRole[] = ['customer', 'barber', 'admin'];

const inputCls = 'w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder-gray-400 outline-none focus:border-ocean-500 focus:ring-2 focus:ring-ocean-500/20 transition';
  ...
  return (
    <div>
      <div className="flex items-start justify-between mb-5">
        <div>
          <h2 className="text-lg font-bold text-gray-900">Users & Barbers</h2>
        ...
        <button
          ...
          className="rounded-xl bg-brand-500 px-4 py-2 text-sm font-bold text-white transition hover:bg-brand-600 shrink-0"
        >
      ...
      {/* add form */}
      {showAdd && (
        <div className="mb-5 rounded-xl border border-ocean-200 bg-ocean-50 p-5">
          <p className="text-xs font-bold uppercase tracking-widest text-ocean-600 mb-4">New user</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Field label="Full name *">
              <input type="text" value={addForm.full_name} placeholder="Full name" className={inputCls}
                onChange={(e) => setAddForm({ ...addForm, full_name: e.target.value })} />
            </Field>
            <Field label="Email *">
              <input type="email" value={addForm.email} placeholder="email@example.com" className={inputCls}
          ...
          <div className="mt-4">
            <button
              ...
              className="rounded-lg bg-brand-500 px-5 py-2 text-sm font-bold text-white transition hover:bg-brand-600 disabled:opacity-40 flex items-center gap-2"
            >
              {addBusy && <Spinner />}
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {users.map((user) => {
            ...
            return (
              <div key={user.id} className="rounded-xl border border-gray-200 bg-white overflow-hidden shadow-sm">
                ...
                <div className="flex flex-wrap items-center gap-3 px-5 py-4">
                  ...
                  <div className="flex gap-1.5 shrink-0">
                    ...
                    <button
                      ...
                      className="rounded-lg border border-red-200 px-3 py-1 text-xs font-semibold text-red-500 transition hover:bg-red-50 disabled:opacity-40"
                    >
                ...
                {isViewing && (
                  <div className="border-t border-gray-100 bg-gray-50 px-5 py-4">
                    ...
                    {user.role === 'barber' && (
                      <>
                        <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">Available hours</p>
                ...
                {isEditing && editForm && dayState && (
                  <div className="border-t border-gray-100 bg-gray-50 px-5 py-4">
                    <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-4">Edit user</p>
        ...
        return (
          <div
            ...
            className={`flex flex-wrap items-center gap-3 rounded-lg border px-4 py-2.5 transition-colors ${
              day.enabled ? 'border-ocean-200 bg-ocean-50' : 'border-gray-200 bg-white'
            }`}
          >
            <label className="flex items-center gap-2.5 cursor-pointer w-16 shrink-0">
              ...
              />
              <span className={`text-xs font-bold uppercase tracking-wider ${day.enabled ? 'text-gray-900' : 'text-gray-400'}`}>
                {label}
              </span>
            </label>
            ...
            {day.enabled ? (
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="rounded-md border border-gray-300 bg-white px-2 py-1 text-xs text-gray-900 outline-none focus:border-ocean-500"
                />
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
      {DAYS.map(({ key, label }) => {
        const h = hours[key];
        return (
          <div key={key} className={`rounded-lg border px-3 py-2 text-xs ${h ? 'border-gray-200 bg-white' : 'border-gray-100 opacity-50'}`}>
            <p className="font-bold uppercase tracking-wider text-gray-400 mb-0.5">{label}</p>
            {h ? (
              <p className="text-gray-700">{h[0]} – {h[1]}</p>
            ) : (
...
function RoleSelect({ value, onChange }: { value: UserRole; onChange: (r: UserRole) => void }) {
  return (
    <select
      ...
      className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 outline-none focus:border-ocean-500 transition"
    >
      {ROLE_OPTIONS.map((r) => (
...
function DetailItem({ label, value, mono = false, className = '' }: { label: string; value: string | null | undefined; mono?: boolean; className?: string }) {
  return (
    <div className={className}>
      <dt className="text-xs text-gray-400 uppercase tracking-wider font-semibold mb-0.5">{label}</dt>
      <dd className={`text-gray-700 ${mono ? 'font-mono text-xs' : 'text-sm'}`}>
